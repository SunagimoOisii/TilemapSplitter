...
public class TilemapSplitterWindow : EditorWindow
{
    ...
    public void CreateGUI()
    {
        ...
        CreateFoldout(container, "交差タイル", () => crossOption, v => crossOption = v, () => crossLayer, v => crossLayer = v,
            () => previewCross, v => previewCross = v, () => colorCross, v => colorCross = v,
            "プレビューは縦タイルの設定で操作可能", "プレビューは横タイルの設定で操作可能", "Ignore では何も生成されないのでプレビューもない");

        CreateFoldout(container, "T字タイル", () => tOption, v => tOption = v, () => tLayer, v => tLayer = v,
            () => previewT, v => previewT = v, () => colorT, v => colorT = v,
            "プレビューは縦タイルの設定で操作可能", "プレビューは横タイルの設定で操作可能", "Ignore では何も生成されないのでプレビューもない");

        CreateFoldout(container, "角タイル", () => cornerOption, v => cornerOption = v, () => cornerLayer, v => cornerLayer = v,
            () => previewCorner, v => previewCorner = v, () => colorCorner, v => colorCorner = v,
            "プレビューは縦タイルの設定で操作可能", "プレビューは横タイルの設定で操作可能", "Ignore では何も生成されないのでプレビューもない");

        CreateFoldout(container, "孤立タイル", () => isolateOption, v => isolateOption = v, () => isolateLayer, v => isolateLayer = v,
            () => previewIsolate, v => previewIsolate = v, () => colorIsolate, v => colorIsolate = v,
            "プレビューは縦タイルの設定で操作可能", "プレビューは横タイルの設定で操作可能", "Ignore では何も生成されないのでプレビューもない");

        AddSeparator(container);

        CreateFoldout(container, "縦エッジ", () => generateVerticalEdge ? ClassificationOption.Independent : ClassificationOption.Ignore,
        ...
        CreateFoldout(container, "横エッジ", () => generateHorizontalEdge ? ClassificationOption.Independent : ClassificationOption.Ignore,
